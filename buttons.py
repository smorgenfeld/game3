...
def buttonChoice3(p1x,p1y,p2x,p2y,text1,text2,text3,win,undraw,b1e=True,b2e=True,b3e=True):
    ...
    while i == False:
        x=win.getMouse()
        if b1e and x.getX() >= p1x and x.getX() <= (p1x+p2x)/2-((p1x+p2x)/40) and x.getY() >= (p1y+p2y)/2+20 and x.getY() <= p2y:
                rekt1.setFill('grey')
                win.update()
                sleep(.2)
                rekt1.setFill('white')
                ...
                if undraw == True:
                    ...
                    win.update()
                return 1
        elif b2e and x.getX() >= (p1x+p2x)/2+((p1x+p2x)/40) and x.getX() <= p2x and x.getY() >= (p1y+p2y)/2+20 and x.getY() <= p2y:
                rekt2.setFill('grey')
                win.update()
                sleep(.2)
                rekt2.setFill('white')
                ...
                if undraw == True:
                    ...
                    win.update()
                return 2
        elif b3e and x.getX() >= p1x+((p2x-p1x)/4) and x.getX() <= p1x+((3*(p2x-p1x))/4) and x.getY() >= p1y and x.getY() <= (p2y+p1y)/2-20:
                rekt3.setFill('grey')
                win.update()
                sleep(.2)
                rekt3.setFill('white')
