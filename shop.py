...
def shop(win,screen):
    global back,forwards,op1,op2,op3,op4,op5,op6,op7,op8,op9,op10, unlocks, backback, getout
    getout = False
    #get the save data
    ...
    l1 = Text(Point(960,1000),'');l1.setFace('helvetica');l1.setSize(30);l1.draw(win)
    ...
    Text(Point(150,550),chr(8592)).draw(win)
    Text(Point(1750,550),chr(8594)).draw(win)
    ...
    if screen == 1:
        ...
        l1.setText('Shop: Characters')
    elif screen == 2:
        l1.setText('Shop: Upgrades')
    ...
    while yay:
        ...
        for i in range(len(clicks)):
            ...
            if yay != True:
                overlay(sel,costs,caish,names,selected_skin,high_score,win,clicks,screen)
            if getout:
                return
